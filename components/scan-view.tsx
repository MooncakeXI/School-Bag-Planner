...
export function ScanView({ items }: { items: ScanItem[] }) {
  ...
  const handleDecoded = useCallback(async (code: string) => {
  ...
  return (
    <div className="mx-auto flex w-full max-w-lg flex-col overflow-hidden rounded-3xl bg-[#141110]">
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-[#221C18]">
        <video ref={videoRef} muted playsInline className="h-full w-full object-cover" />
        <canvas ref={canvasRef} hidden />
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="size-56 rounded-2xl border-4 border-white/80" />
        </div>
        <p className="absolute inset-x-0 top-4 text-center text-sm text-white/75">
          เล็งสติกเกอร์ QR บนปกหนังสือ
        </p>
